...
```mermaid
flowchart TB
  Up[rpa client up] --> Bootstrap[launchctl bootstrap]
  Bootstrap --> Ready{IPC socket ready?}
  Ready -- yes --> Healthy[service running]
  Ready -- no --> PrintState[launchctl print + recent log tail]

  Status[rpa status] --> IPC{IPC reachable?}
  IPC -- yes --> LiveState[live status]
  IPC -- no --> FallbackState[launchd state + persisted last error]

  Logs[rpa logs client] --> IPCLogs{IPC logs reachable?}
  IPCLogs -- yes --> RingBuffer[in-memory ring logs]
  IPCLogs -- no --> FileTail[~/.rpa/logs/client.log tail]
```

```mermaid
sequenceDiagram
  participant U as User
  participant CLI as rpa CLI
  ...
  LD->>P: spawn
  P-->>L: startup logs
  CLI->>CLI: readiness check (IPC)
  alt IPC unavailable
    CLI->>LD: print job state
    CLI->>L: tail recent lines
  end
```
